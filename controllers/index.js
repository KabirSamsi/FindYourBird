...
const {sortByPopularity} = require('../utils/popularityAlgorithms');
...
module.exports.identifyForm = function(req, res) {
  return res.render('identify', {birdInfo: false, colors, habitats, sizes});
}

module.exports.identify = async function(req, res) {
  let allowed_sizes = [];
...
module.exports.showBird = async function(req, res) {
  const bird = await Bird.findById(req.params.id);
  if (!bird) {
...
module.exports.updateBird = async function(req, res) {
  let finalHabitats = [];
  for (let habitat of habitats) {
    if (req.body[habitat] == 'on') {
      finalHabitats.push(habitat);
